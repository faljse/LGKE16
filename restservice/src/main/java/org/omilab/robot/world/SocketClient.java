...
public class SocketClient {
	 ...
	 * @param mode
	 */
	private void receiveFile (String escapedPathAndFilename, byte mode, int portoffset) {
		Socket soc = null;
		Socket soc2 = null;
		...
		try {
			...
			if (soc.getInputStream().read() == 0)
				System.out.println("Error in sendFile command, check server status!");
	...
	private void sendFile(String path, byte mode, int portoffset) {
		...
		try {
			...
			if (soc.getInputStream().read() == 0)
				System.out.println("Error in receiveImage command, check server status!");
	 ...
	 */
	public void receiveSound(String imagePath) {
